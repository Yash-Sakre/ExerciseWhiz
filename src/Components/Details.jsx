  ...
  return (
    <div className=" mx-[1rem] bg-white mt-10  lg:mx-[3rem] min-w-[90vw] items-center rounded-2xl shadow-lg shadow-red-100 my-auto px-[2rem] py-[2rem] flex flex-col lg:flex-row justify-between">
      ...
      <div className="flex flex-col lg:flex-col">
        <div className=" text-[2rem] lg:text-[2.5rem] font-bold lg:w-[50vw] capitalize">{name}</div>
        <div className="lg:w-[40vw] font-semibold ">
          Staying active with regular workouts is essential for maintaining
          strength and fitness. Incorporating the <span className="text-primary">{name}</span> exercise into your
          routine can effectively target your <span className="text-primary">{target}</span> muscles, and it has the
        ...
        {ExtraDetails.map((item,index) => (
          <div key={index} className="flex items-center gap-5 ">
            ...
            <div>
            <p className=" capitalize text-[1.2rem] font-semibold">{item.name}</p>
